...
WASD movement was one of the first features I added in Cycle 1. I made each character have a different movement speed later in Cycle 11, but WASD movement has remained largely unchanged. The player can use the WASD keys to move the player around the level in the corresponding direction (up, left, down, right).

#### Criterion 2 - Achieved

Dashing is another feature which has been in the game since the first cycle. The player can use the right mouse button to momentarily increase the player's movement speed and there is a short cooldown before they can do this again. In Cycle 11, I changed dashing so that the statistics are different for each playable character and in Cycle 15 I added a dash cooldown bar so it is clear for players when they can dash again.

#### Criterion 3 - Achieved

In Cycle 2 I added a basic weapon which fired a bullet towards the cursor when the mouse was clicked. I later added different weapons with unique statistics in cycles 10 and 13. Weapons vary with different reload times, bullet damage, bullet speed, accuracy and unlock cost. Some weapons are also automatic (you can hold down the mouse to repeatedly fire), which is a feature I added in Cycle 16. I've always kept shooting simple because it is one of the game's core components so should remain as easy as possible.

#### Criterion 4 - Not developed

Initially, I planned to use the E key to interact with chests and purchasable items. However, I ultimately decided not to include chests in my game and to implement the shop differently than previously planned. I will explain why later in my analysis, but there is now no need to interact using the E key.

#### Criterion 5 - Achieved

In Cycle 10, I added multiple weapons which could be switched between using the number keys. When each number key is pressed, the code checks if you own that weapon before trying to switch to it. This prevents any errors. When I added more weapons in Cycle 13, it was as simple as adding some more number keys to accommodate every weapon.
...
I added the shop feature in Cycle 13. Players can touch the shopkeeper to see the list of items and then press the corresponding key to buy that item if they have enough coins. In Cycle 16 I fixed a few oversights with the shop feature which could cause some bugs. Initially, I had planned to implement the shop feature in a different way.

#### Criterion 12 - Not developed

During development, I decided not to implement chests in my game because I felt that it would not improve gameplay and wasn't needed. Players already received enough coins from killing enemies to buy weapons from the shop so it would make the game too easy. Also, it would mean there would be an extra room per floor without any combat, which could have made the gameplay feel a bit slow.

#### Criterion 13 - Achieved

I added spikes and boxes in the tilemap for levels in Cycle 3 but it wasn't until Cycle 5 that the spikes could damage the player and Cycle 12 for projectiles to destroy the boxes. Also in Cycle 12, I made proper level tile maps for the game, however, I included too many spikes and boxes so I later had to reduce their density. When there are too many spikes and boxes, the game becomes too difficult since it is too hard to move around without taking damage, and it can feel unfair as there is nothing that you can do about it.
...
In Cycle 12, I changed level generation so that the random levels for each floor are chosen from 3 different lists, one for each floor. This meant that I could set different enemy spawns for the rooms on each floor. I made the enemies harder as you increase floors. This adds a sense of progression and challenge to the game which keeps it engaging.

#### Criterion 15 - Achieved

I added proper enemies in Cycle 8b. They move for a set time before pausing to shoot a bullet and repeating the cycle. This allows players to somewhat predict the enemy movements which allows players to become skilled at the game and learn how to get further each time. I properly balanced the game in Cycle 19, which is where I adjusted the statistics for each enemy to make them feel different, for example, some will move faster than others. All enemies, however, follow the same pattern.

#### Criterion 16 - Achieved

Boss fights were added in Cycle 14, and the health bar for the boss was added in Cycle 16. There is a boss fight at the end of each floor and the bosses are stronger on higher floors. Stronger bosses have more health and stronger attacks. The first boss shoots normally, the second boss shoots in fast bursts of 5 and the third boss shoots in a shotgun arc. Bosses pose a final challenge for each floor before players can spend their coins in the shop.

#### Criterion 17 - Achieved

When adding boss fights in Cycle 14, one of the boss' abilities is to spawn additional enemies to attack the player. Stronger bosses will spawn stronger enemies but will have a slightly lower chance of spawning them after shooting.
...
When I built the level tile map in Cycle 3, I used the solid property on the walls so that the player couldn't walk through them. I used this again when adding enemies and bosses so that they can collide with the player too.

#### Criterion 19 - Achieved

Whenever a player bullet collides with an enemy, the bullet is destroyed and the `updateHealth` function is called inside the enemy class which applies the bullet's damage and then calls another function which checks if the health is equal to or below 0. Collision detection uses the `onCollide` function within Kaboom. This functionality was added as part of Cycle 8b when I implemented a working enemy system into the game.

#### Criterion 20 - Achieved

The `destroy` function inside the enemy class is called whenever the enemy takes damage. The destroy function checks if the health is below or equal to 0 and destroys the enemy if it is. I added this in Cycle 8b as part of how enemies behave.

#### Criterion 21 - Achieved

I added coin drops in Cycle 9 along with a coin counter which displays how many coins the player has. Whenever an enemy is killed, the coin count is incremented by 1. Originally I planned for enemies to drop coins on the floor which players had to pick up but during development, I realised that that was unnecessarily complicated. Initially, when I added bosses in Cycle 14, they awarded one coin too, but in Cycle

#### Criterion 22 - Not developed

I haven't included powerups in my game due to time constraints. I would have liked to include this feature but I decided to spend my time on more critical features instead. Initially, I planned to sell permanent and temporary powerups in the shop and also have them as chest rewards, however now the way I would implement powerups has changed. I think powerups would fit better into my game if there was a rare chance for a powerup to spawn when a box is destroyed and if they were temporary only. This would make boxes feel more useful and be rewarding for the player as it gives them additional functionality. I will discuss what I would like to have done with powerups later in [Maintenance and Future Development](4.3-maintenance-and-future-development.md).

#### Criterion 23 - Not developed

See the above criterion.

#### Criterion 24 - Achieved

In Cycle 5 I added the player health bar and damage system, however, there were no enemies yet so the only way for the player to get damaged was by touching spikes. It was quite useful that I hadn't added enemies yet because it allowed me to develop the player health system without worrying if enemies were working. Damage from spikes has remained largely unchanged since Cycle 5, the only change to it has been the addition of the effect when taking damage which I added in Cycle 17.

#### Criterion 25 - Achieved

I added health replenishment as part of the shop in Cycle 13. If the player had enough coins they could press l to restore a portion of their health. This added an interesting dynamic to the game because when the player comes to the shop they have to decide if they want to prioritise restoring health or buying new weapons.
...
I added the initial main menu and character selection in Cycle 11 but it was pretty basic and needed changes so I improved it in Cycle 15. In Cycle 18, I redesigned the menu buttons and polished off the menu and character selection to ensure a pleasant user experience.

#### Criterion 27 - Achieved

&#x20;I added the pause menu in Cycle 17, which allows players to pause and unpause the game using the escape key. It adds a white overlay to the screen and in Cycle 18, I made the music get quieter too. These are examples of feedback to the user which should make the game feel nicer to play.

#### Criterion 28 - Achieved

The player's health bar has remained largely unchanged since its implementation in Cycle 5. The health bar scales to the player's current health so whenever the player takes damage the health bar decreases too. When I added more characters in Cycle 11, I changed the health bar so that the starting width would correspond to the health of the chosen character.

#### Criterion 29 - Not developed

I haven't included a powerup display as I wasn't able to implement a powerup system into my game. See the description for Criterion 22 for details.

#### Criterion 30 - Not developed

During development, I decided not to add this feature because I felt that it was unnecessarily complicated. The existing inventory system in my game is easy to understand and there isn't really a need for players to change the keys for their weapons because, in a typical game, you'll only unlock 2 to 3 weapons. Therefore it will always be easy to switch weapons as the 1, 2 and 3 keys are near to where your left hand will be for movement anyway.
...
The guidelines for a PEGI-12 rating are "_Video games that show violence of a slightly more graphic nature towards fantasy characters or non-realistic violence towards human-like characters would fall in this age category. Sexual innuendo or sexual posturing can be present, while any bad language in this category must be mild."_ ([PEGI, 2023](reference-list.md#2.1-design-frame)) My game meets these requirements as it only contains mild fantasy violence and has no sexual content or bad language.
