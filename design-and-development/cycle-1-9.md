...
| Variable Name                      | Use                                                                                                                                                                                                         |
| ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `possibleLevels` and `fixedLevels` | These arrays hold level configurations including layout and enemy spawns for both dynamic (possible) levels and fixed levels.                                                                               |
...
| `chosenLevels`                     | An array where selected levels are stored. It's populated with fixed levels, random possible levels, and then another fixed level for each iteration in the outer loop.                                     |
...
| `player`                           | The object representing the player entity within the game. It's used for calculating target positions for enemy movement and shooting.                                                                      |
...
| ---- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| 1    | Start first level with 't'.            | <ul><li>Enemies spawn at the correct locations.</li><li>Enemies move towards the player.</li><li>Enemies pause to shoot before moving again.</li></ul> | As expected. (Note that some spawn locations are outside of the level so when making proper game levels I must ensure that spawn locations are within bounds.) | Pass.     |
| 2    | Move the player around with WASD keys. | <ul><li>Enemies keep moving towards the new player location, following the player</li><li>Enemies shoot correctly at the new player location</li></ul> | As expected.                                                                                                                                                   | Pass.     |
| 3    | Increment levels with 'r'.             | <ul><li>Enemies and bullets from the previous level are deleted</li><li>New enemies spawn, move and shoot correctly</li></ul>                          | As expected.                                                                                                                                                   | Pass.     |
